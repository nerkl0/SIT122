...
const float OBSTACLE_SETPOINT = 15.0;
const float OBSTACLE_MIN_DISTANCE = 10.0;  
const float TURN_SCALE = 0.04;
...
void turnBot(int speed){
  ...
  if (mBot_state == LEFT) leftMotor; 
  else if (mBot_state == RIGHT) rightMotor;
...
/*
  findBot attempts to find the line again if both sensors have lost it. 
...
void findBot(){
  ...
  while ((SensorState)lineFinder.readSensors() == OFF_LINE && millis() - start < stop_search){
    _loop();
    moveBot(TURN_SPEED * 1.2, TURN_SCALE);
  }
...
/*
  Turns the robot ~35deg clockwise moving slightly away from the obstacle.
  Then performs a gradual arc around the object by independently scaling each motor across 35 steps. 
  The left wheel increases the multiplier from 0.4 - 0.8; Right wheel decreses scaling from 1.5 to 1.1.
  FindBot is then called to re-align if necessary 
*/
void avoidBot(){
  Serial.println("!! Obstacle Detected !!");
  stopBot(); 

  // turn away from object and move slightly forward
  turnBot(TURN_SPEED);
  _delay(0.4);
  stopBot(); 
  moveBot(TURN_SPEED, 1.0);
  _delay(0.3);
  stopBot(); 
  ...
  for(int i = 0; i < steps; i++){
    ...
    motor_l.setMotorPwm(TURN_SPEED * (0.4 + 0.4 * arc));
    motor_r.setMotorPwm(-TURN_SPEED * (1.5 - 0.4 * arc));
    _loop();
    if (lineFinder.readSensors() != OFF_LINE) break;  // break early if a line is found 
    _delay(0.05);
  }

  stopBot();
  updateState(LEFT, L_YELLOW);
  findBot();
}
...
void loop() {
  ...
  // This if block is triggered after findBot has unsuccesfully found the line after searching
